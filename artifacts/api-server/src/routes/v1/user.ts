...
router.get("/stats", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [rank, totalUsers, rpgRow, guildRow, bankCapItems] = await Promise.all([
      ...
      col("rpg_characters").findOne({ user_id: user.id }),
...
router.get("/avatar", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (user.profile_picture) {
    const buf = Buffer.isBuffer(user.profile_picture) ? user.profile_picture : Buffer.from(user.profile_picture, "base64");
...
router.get("/background", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (user.profile_background) {
    const buf = Buffer.isBuffer(user.profile_background) ? user.profile_background : Buffer.from(user.profile_background, "base64");
    res.set("Content-Type", "image/jpeg");
    res.set("Cache-Control", "no-cache");
    res.send(buf);
    return;
...
router.get("/achievements", requireAuth, async (req: AuthRequest, res) => {
  try {
    const achievements = await col("web_achievements")
