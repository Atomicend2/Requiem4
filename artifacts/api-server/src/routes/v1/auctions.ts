...
router.post("/:id/bid", requireAuth, async (req: AuthRequest, res) => {
  ...
  const phone = (user as any).phone || extractNumberFromJid(String((user as any).id || ""));
  const dbUser = await col("users").findOne({ $or: [{ phone }, { wa_sender: phone }] } as any);
  const balance = (dbUser as any)?.balance ?? 0;
  ...
  const bidderId = phone ? `${phone}@s.whatsapp.net` : String((user as any).id || "");
