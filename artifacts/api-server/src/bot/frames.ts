...
export async function seedTensuraFrames(): Promise<{ added: number; skipped: number }> {
  ...
  for (const entry of entries) {
    ...
    // Derive a readable name from the filename, e.g. "frames/42.png" → "Tensura Frame 42"
    const fileNum = url.split("/").pop()?.replace(/\.\w+$/, "") || "";
    const name = `Tensura Frame ${fileNum}`;

    const result = await col("frames").updateOne(
      { url, uploaded_by: "system" },
      {
        $setOnInsert: {
          name,
          theme: "tensura",
  ...
  logger.info({ added, skipped, total: entries.length }, "Tensura community frame seed complete");
