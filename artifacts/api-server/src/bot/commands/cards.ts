...
import { getTierEmoji, formatNumber, generateId, VIDEO_TIERS, isGifBuffer, mentionTag } from "../utils.js";
...
export async function handleCards(ctx: CommandContext): Promise<void> {
  ...
  if (cmd === "card") {
    ...
    await sendMedia(from, buf, VIDEO_TIERS.has(c.tier) || c.is_animated === 1 || c.is_animated === true, caption);
    } catch (err) {
  ...
  if (cmd === "cardinfo" || cmd === "ci") {
    ...
    const tierMap: Record<string, string> = {
      "1":"T1","2":"T2","3":"T3","4":"T4","5":"T5","6":"T6","s":"TS","x":"TX","z":"TZ",
      "c":"T2","r":"T4","sr":"T5","ssr":"T6","ur":"TS",
    };
    const lastArg = args[args.length - 1].toLowerCase();
    let searchTier: string | null = null;
    let nameParts = args;
    if (tierMap[lastArg]) { searchTier = tierMap[lastArg]; nameParts = args.slice(0, -1); }
    ...
    const nameRegex = new RegExp(`\\b${escapedSearch}\\b`, "i");
    const allMatches = (allCards as any[]).filter((c) => nameRegex.test(c.name) && (searchTier ? c.tier === searchTier : true));
    if (allMatches.length === 0) { await sendText(from, "❌ Card not found."); return; }
    const MAX_CI_RESULTS = 3;
    const matches = allMatches.slice(0, MAX_CI_RESULTS);
    if (allMatches.length > MAX_CI_RESULTS) {
      await sendText(from, `🔍 Found ${allMatches.length} matches — showing first ${MAX_CI_RESULTS}. Be more specific to narrow results.`);
    }
    for (let i = 0; i < matches.length; i++) {
      const found = matches[i];
      try {
      const owners = await getCardOwners(found.id);
      ...
      if ((owners as any[]).length > 0) {
        ...
        ownersSection = shown.map((o: any) => {
          ...
          const u = { display_id: extractNumberFromJid(o.user_id) };
          return `• #${o.issue_num} ${u.display_id} [${o.copy_id || o.user_card_id}]`;
        }).join("\n");
      ...
      await sendMedia(from, buf, isAnimated, caption, ownerMentions);
      } catch (err) {
  ...
  if (cmd === "ss") {
    ...
    const allCards = await getAllCards();
    const seriesCards = (allCards as any[]).filter((c) => (c.series || "General").toLowerCase().includes(seriesName.toLowerCase()));
    ...
    for (let i = 0; i < seriesCards.length; i++) { text += `├─ 🃏 ${i + 1}. ${seriesCards[i].name}\n│   ᴛɪᴇʀ: ${seriesCards[i].tier}\n`; }
    text += `╰──────────────╯`;
    await sendText(from, text);
    return;
  }
  ...
  if (cmd === "cardleaderboard" || cmd === "cardlb") {
    ...
    (lb as any[]).forEach((e, i) => {
      ...
      const name = extractNumberFromJid(e.user_id);
  ...
  if (cmd === "bid") {
    ...
    if (!shortId || !amountStr) {
      await sendText(from, "❌ Usage: .bid <auction_id> <amount>\n_Example: .bid abc123 7500_"); return;
    }
    const amount = parseInt(amountStr, 10);
    if (isNaN(amount) || amount < 1) { await sendText(from, "❌ Invalid bid amount."); return; }

    const auctions = await getAuctionsLive();
    const match = auctions.find((a: any) => String(a.id).slice(-6) === shortId);
    if (!match) { await sendText(from, "❌ Auction not found. Use .auctions to see active auctions."); return; }

    const bidderUser = await getUser(userId);
    const balance = (bidderUser as any)?.balance ?? 0;
    if (balance < amount) {
      await sendText(from, `❌ Insufficient coins.\n💰 Your balance: $${formatNumber(balance)}\n🎯 Bid amount: $${formatNumber(amount)}`); return;
    }

    const bidderName = (bidderUser as any)?.name || userId;
    const result = await placeBid(match.id, sender, bidderName, amount);
    if (!result.ok) {
      await sendText(from, `❌ ${result.message}`); return;
    }
    await sendText(from, `✅ *Bid Placed!*\n\n🎴 *${match.card_name}* [${match.card_tier}]\n💰 Your bid: $${formatNumber(amount)}\n⚔️ Beat by bidding higher before the auction ends!\n\n_ID: \`${shortId}\`_`);
    return;
  }
  ...
  if (cmd === "claim") {
    ...
    const TIER_CLAIM_PRICES: Record<string, number> = { T1:500,T2:1000,T3:2500,T4:5000,T5:10000,T6:15000,TS:25000,TX:50000,TZ:100000 };
    const claimCost = TIER_CLAIM_PRICES[(cardData as any).tier] || 500;
    ...
    await updateUser(userId, { balance: claimerBal - claimCost });
    try {
      const buf = await getCardImageBuffer(cardData);
      const isAnim = VIDEO_TIERS.has((cardData as any).tier) || (cardData as any).is_animated === 1 || (cardData as any).is_animated === true;
      const claimCaption = `🎉 ${mentionTag(sender)} claimed *${(cardData as any).name}* (${(cardData as any).tier})! 💰 -$${formatNumber(claimCost)}`;
      await sendMedia(from, buf, isAnim, claimCaption, [sender]);
    } catch (err) {
      logger.error({ err, card: (cardData as any)?.name }, "Failed to send claimed card media — confirming via text instead");
      await sendText(from, `🎉 ${mentionTag(sender)} claimed *${(cardData as any).name}* (${(cardData as any).tier})! 💰 -$${formatNumber(claimCost)}\n\n_(card image failed to send — use .ci to view it)_`, [sender]).catch(() => {});
    }
    return;
  }
  ...
  if (cmd === "slb") {
    ...
    const rows = await col("user_cards").aggregate([
      ...
      { $unwind: "$card" },
      { $match: { "card.series": { $regex: seriesName, $options: "i" } } },
...
async function getCardImageBuffer(card: any): Promise<Buffer> {
  ...
  if (!mediaUrl && card.raw_data) {
    try { const raw = typeof card.raw_data === "string" ? JSON.parse(card.raw_data) : card.raw_data; mediaUrl = raw?.media_url || null; } catch {}
  }
  ...
  if (mediaUrl) {
    try {
      const res = await fetch(mediaUrl, { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" }, signal: AbortSignal.timeout(15000) });
      if (res.ok) {
        const contentType = res.headers.get("content-type") || "";
        const contentLength = Number(res.headers.get("content-length") || 0);
        const MAX_MEDIA_BYTES = 20 * 1024 * 1024; // 20 MB cap
        if (contentLength > MAX_MEDIA_BYTES) {
          // Too large — fall through to SVG placeholder
        } else {
          const buf = Buffer.from(await res.arrayBuffer());
          if (buf.length > MAX_MEDIA_BYTES) {
            // Still too large after download — fall through
          } else if (contentType.includes("gif") || contentType.includes("webm") || contentType.includes("video")) {
            return buf;
          } else {
            try {
              return await sharp(buf).jpeg({ quality: 88 }).toBuffer();
            } catch (sharpErr) {
              // Malformed/unsupported image data — fall through to SVG placeholder
              // instead of letting a native libvips error take down the process.
            }
          }
        }
      }
  ...
  const svg = `<svg width="900" height="1260" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#111827"/><stop offset="55%" stop-color="#312e81"/><stop offset="100%" stop-color="#020617"/></linearGradient></defs><rect width="900" height="1260" rx="42" fill="url(#bg)"/><rect x="54" y="54" width="792" height="1152" rx="32" fill="none" stroke="#eab308" stroke-width="10"/><text x="450" y="560" fill="#fde68a" font-size="82" font-family="Arial" font-weight="700" text-anchor="middle">${escapeSvg(card.name || "Unknown Card")}</text><text x="450" y="680" fill="#dbeafe" font-size="48" font-family="Arial" text-anchor="middle">${escapeSvg(card.series || "General")}</text><text x="450" y="930" fill="#f8fafc" font-size="72" font-family="Arial" font-weight="700" text-anchor="middle">${escapeSvg(card.tier || "T?")}</text></svg>`;
