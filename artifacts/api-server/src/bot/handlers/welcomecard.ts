...
async function render(opts: WelcomeCardOpts): Promise<Buffer> {
  ...
  const overlaySvg = `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Headline -->
    <text x="230" y="110" font-family="Arial,Helvetica,sans-serif" font-size="42" font-weight="900"
          fill="${accent}" filter="url(#glow)" letter-spacing="6">${headline}</text>

    <!-- Name -->
    <text x="230" y="162" font-family="Georgia,serif" font-size="30" font-weight="700"
          fill="white" paint-order="stroke" stroke="rgba(0,0,0,.7)" stroke-width="4"
          stroke-linejoin="round">${name}</text>

    <!-- Subline -->
    <text x="230" y="200" font-family="Arial,Helvetica,sans-serif" font-size="17"
          fill="rgba(255,255,255,0.65)">${subline}</text>

    <!-- Member count -->
    ${memberLine
      ? `<text x="230" y="228" font-family="Arial,Helvetica,sans-serif" font-size="14"
    ...
    <text x="${W - 16}" y="${H - 10}" text-anchor="end"
          ...
          fill="rgba(255,255,255,0.12)" letter-spacing="3">REQUIEM ORDER 反逆</text>
  </svg>`;
