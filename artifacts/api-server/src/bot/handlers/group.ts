...
export async function handleGroupParticipantsUpdate(
  ...
  logger.info({ groupId, participants, action }, "group-participants.update received");
  ...
  const freshGroup = (await getGroup(groupId)) || group;
  ...
  for (const rawParticipant of participants) {
    ...
    if (action === "add") {
      ...
      if (freshGroup?.welcome === "on") {
        ...
        const pushName = (groupMeta?.participants as any[] | undefined)?.find((p: any) => p.id === participant)?.name;
        ...
        try {
          ...
          if (card) {
            ...
            await sendText(groupId, msg, [participant]);
          }
        } catch (err) {
          logger.error({ err, groupId, participant }, "Welcome send failed — retrying plain text");
          try { await sendText(groupId, msg, [participant]); } catch {}
        }
      }
      ...
      if (freshGroup?.leave === "on") {
        const pushName = (groupMeta?.participants as any[] | undefined)?.find((p: any) => p.id === participant)?.name;
        const displayName = pushName || participant.split("@")[0].split(":")[0];
        const memberCount = Math.max(0, (groupMeta?.participants?.length ?? 1) - 1);
